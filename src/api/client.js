...
// Mirrors the client/dev pattern: bearer access token in memory
// + httpOnly refresh cookie scoped to /api/v1/admin, with
...
async function rawRequest(path, { method = 'GET', body, headers = {}, skipAuth = false } = {}) {
  ...
  if (!res.ok) {
    throw new ApiError(data?.message || 'Nomalum xatolik yuz berdi.', res.status, data?.errorCode);
  }
  return data;
}
...
async function refreshAccessToken() {
  if (!refreshPromise) {
    refreshPromise = rawRequest('/admin/auth/refresh', { method: 'POST', skipAuth: true })
      .then((data) => { setAccessToken(data.accessToken); return data.accessToken; })
      .catch((err) => { setAccessToken(null); throw err; })
